...
export class WalletTracker {
    ...
    *    transactions. If it finds a transaction made from a tracked wallet, then forward the transaction to a transaction handler.
    ...
    createWalletGroupsMap(groups: IGroup[]):  Map<string, Set<string>>{
        ...
        groups.forEach((g) => { 
            [...g.wallets.keys()].forEach((w) => {
                const wAddress = w.toLowerCase();
                if(walletGroupsMap.has(wAddress)) {
                    const groupsForWallet = walletGroupsMap.get(wAddress);
                    if(!(groupsForWallet?.has(g.name))){
                        groupsForWallet?.add(g.name);
                    }
                }
                else {
                    walletGroupsMap.set(wAddress, new Set<string>());
                    walletGroupsMap.get(wAddress)?.add(g.name);
                };
            })
        });
